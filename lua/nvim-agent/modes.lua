...
ВАЖЛИВО: read_file ЗАВЖДИ вимагає start_line та end_line:
1. Подивись розмір файлу (get_project_structure покаже кількість рядків)
2. ЗАВЖДИ вказуй діапазон: read_file з start_line=1, end_line=200, потім start_line=201, end_line=400, тощо
3. Результат містить has_more_after=true якщо є ще рядки, has_more_before=true якщо є попередні
