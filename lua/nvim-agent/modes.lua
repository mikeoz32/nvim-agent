...
📁 Робота з файлами:
- read_file - читання вмісту файлу (для великих файлів >200 рядків використовуй start_line/end_line для читання частинами)
...
- Якщо треба знайти щось у коді - використай grep_search або find_files
- Якщо треба прочитати файл - СПОЧАТКУ дізнайся його розмір, ПОТІМ читай частинами якщо він великий (>200 рядків)
- Якщо треба відредагувати - використай write_file або replace_text

ВАЖЛИВО: Для великих файлів (>200 рядків) ЗАВЖДИ читай частинами:
1. Подивись розмір файлу (get_project_structure покаже кількість рядків)
2. Читай по 100-200 рядків за раз: read_file з start_line=1, end_line=200, потім start_line=201, end_line=400, тощо
3. Результат містить has_more=true якщо є ще рядки
